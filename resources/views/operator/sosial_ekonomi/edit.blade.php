        ...
        <div class="w-4/5 max-w-full px-3 mt-0 lg:flex-none">
            <div
                ...
                <div class="border-black/12.5 mb-0 rounded-t-2xl border-b border-solid p-6 pt-4 pb-0">
                    <h6 class="capitalize font-semibold">Form Edit Data Sosial Ekonomi</h6>
                </div>
                <div class="flex-auto p-6">
                    <form action="{{ route('sosial-ekonomi.update', $data->id_sosial_ekonomi) }}" method="POST"
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                            <div>
                                ...
                                <select name="nisn" id="nisn" required
                                    class="focus:shadow-primary-outline text-sm block w-full rounded-lg border border-gray-300 bg-gray-100 text-gray-700 px-3 py-2 focus:outline-none cursor-not-allowed"
                                    disabled>
                                    @foreach ($siswa as $item)
                                        <option value="{{ $item->nisn }}" {{ $data->nisn == $item->nisn ? 'selected' : '' }}>
                                            {{ $item->nama }} ({{ $item->nisn }})
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                            <div>
                                <label for="tanggungan" class="block text-sm font-semibold py-2 text-slate-700 mb-1">Jumlah
                                    Tanggungan</label>
                                <input type="number" name="tanggungan" id="tanggungan" value="{{ $data->tanggungan }}"
                                    class="focus:shadow-primary-outline text-sm block w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-gray-700 focus:border-blue-500 focus:outline-none">
                            </div>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="aset" id="aset" value="{{ $data->aset }}"
                                class="focus:shadow-primary-outline text-sm block w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-gray-700 focus:border-blue-500 focus:outline-none">
                        </div>

                        @php
                            $files = ['pkh' => 'PKH', 'dtks' => 'DTKS', 'sktm' => 'SKTM'];
                        @endphp

                        @foreach ($files as $field => $label)
                            <div class="mb-4">
                                <label for="{{ $field }}" class="block text-sm font-semibold py-2 text-slate-700 mb-1">Upload
                                    {{ $label }}</label>
                                ...
                                <span id="{{ $field }}-filename" class="ml-2 text-sm text-gray-600">
                                    {{ $data->$field ? basename($data->$field) : 'Belum ada file' }}
                                </span>

                                <input type="file" name="{{ $field }}" id="{{ $field }}" class="hidden"
                                    onchange="updateFileName('{{ $field }}')">
                            </div>
                        @endforeach
